...
public class ProcessExecutionServiceTests
{
    ...
    public ProcessExecutionServiceTests()
    {
        ...
        A.CallTo(() => _repositories.GetInstance<IProcessStepRepository>())
        ...
        A.CallTo(() => serviceProvider.GetService(typeof(IDimRepositories))).Returns(_repositories);
    ...
    {
        // Arrange
        var processData = _fixture.CreateMany<Guid>().Select(x => new Process(x, ProcessTypeId.SETUP_DIM, Guid.NewGuid())).ToImmutableArray();
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
        ...
        A.CallTo(() => _repositories.SaveAsync())
            .MustHaveHappened(processData.Length * 2, Times.Exactly);
        A.CallTo(() => _repositories.Clear())
            .MustHaveHappened(processData.Length * 2, Times.Exactly);
    }
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLock()
    {
        ...
        var process = new Process(processId, ProcessTypeId.SETUP_DIM, processVersion);
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLockThenThrows()
    {
        ...
        var firstProcess = new Process(firstId, ProcessTypeId.SETUP_DIM, firstVersion);
        var secondProcess = new Process(secondId, ProcessTypeId.SETUP_DIM, secondVersion);
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_UnmodifiedSafeRequested()
    {
        // Arrange
        var firstVersion = Guid.NewGuid();
        var process = new Process(Guid.NewGuid(), ProcessTypeId.SETUP_DIM, firstVersion);
        var processData = new[] { process };
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
            .Returns(processData.ToAsyncEnumerable());
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLock_SaveAsyncThrows()
    {
        ...
        var firstVersion = Guid.NewGuid();
        var firstProcess = new Process(firstProcessId, ProcessTypeId.SETUP_DIM, firstVersion);
        var secondProcessId = Guid.NewGuid();
        var secondVersion = Guid.NewGuid();
        var secondProcess = new Process(secondProcessId, ProcessTypeId.SETUP_DIM, secondVersion);
        var thirdProcessId = Guid.NewGuid();
        var thirdVersion = Guid.NewGuid();
        var thirdProcess = new Process(thirdProcessId, ProcessTypeId.SETUP_DIM, thirdVersion);
    ...
    public async Task ExecuteAsync_IgnoresLockedProcesses_LogsInformation()
    {
        ...
        // Arrange
        var processData = _fixture.CreateMany<Guid>().Select(x => new Process(x, ProcessTypeId.SETUP_DIM, Guid.NewGuid()) { LockExpiryDate = lockExpiryDate }).ToImmutableArray();
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
            .Returns(processData.ToAsyncEnumerable());
