...
using ProcessStepTypeId = Dim.Entities.Enums.ProcessStepTypeId;
using ProcessTypeId = Dim.Entities.Enums.ProcessTypeId;

namespace Dim.Processes.Library.Tests;

public class ManualProcessDataExtensionsTests
{
    private readonly IDimRepositories _respositories;
    ...
    public ManualProcessDataExtensionsTests()
    {
        ...
        _respositories = A.Fake<IDimRepositories>();
        _processStepRepository = A.Fake<IProcessStepRepository>();

        A.CallTo(() => _respositories.GetInstance<IProcessStepRepository>())
    ...
    public void CreateManualProcessData_ReturnsExpected()
    {
        ...
        var processSteps = _fixture.CreateMany<(Guid ProcessStepId, DateTimeOffset Now)>(5).Select(x => new ProcessStep(x.ProcessStepId, _fixture.Create<ProcessStepTypeId>(), Entities.Enums.ProcessStepStatusId.TODO, process.Id, x.Now)).ToImmutableArray();
        ...
        // Act
        var result = sut.CreateManualProcessData(stepTypeId, _respositories, _getProcessEntityName);
        ...
        result.Should().NotBeNull().And.BeOfType<ManualProcessStepData>().And.Match<ManualProcessStepData>(
            data =>
                ...
                data.PortalRepositories == _respositories);
    }
    ...
    public void CreateManualProcessData_WithNullVerifyProcessData_Throws()
    {
        ...
        var Act = () => sut.CreateManualProcessData(_fixture.Create<ProcessStepTypeId>(), _respositories, _getProcessEntityName);
    ...
    public void CreateManualProcessData_WithInvalidProcessStepStatus_Throws()
    {
        // Arrange
        var process = new Process(Guid.NewGuid(), _fixture.Create<ProcessTypeId>(), Guid.NewGuid()) { LockExpiryDate = null };
        var processSteps = _fixture.CreateMany<(Guid ProcessStepId, DateTimeOffset Now)>(5).Select(x => new ProcessStep(x.ProcessStepId, _fixture.Create<ProcessStepTypeId>(), Entities.Enums.ProcessStepStatusId.DONE, process.Id, x.Now)).ToImmutableArray();
    ...
    public void CreateManualProcessData_WithInvalidProcessStepType_Throws()
    {
        ...
        var Act = () => sut.CreateManualProcessData(stepTypeId, _respositories, _getProcessEntityName);
    ...
    public void RequestLock_WithUnLockedProcess_ReturnsExpected()
    {
        ...
        var sut = _fixture.Build<ManualProcessStepData>()
            .With(x => x.Process, process)
            .With(x => x.PortalRepositories, _respositories)
            .Create();
        ...
        A.CallTo(() => _respositories.Attach(process, null)).MustHaveHappenedOnceExactly();
    }
    ...
    public void SkipProcessSteps_ReturnsExpected()
    {
        ...
        var processSteps0 = new ProcessStep[]
            {
                new(Guid.NewGuid(), stepTypeIds[0], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[0], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[0], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before)
            };
        var processSteps1 = new ProcessStep[]
            {
                new(Guid.NewGuid(), stepTypeIds[1], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[1], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[1], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before)
            };
        var processSteps2 = new ProcessStep[]
            {
                new(Guid.NewGuid(), stepTypeIds[2], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[2], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[2], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before)
            };
        var processSteps3 = new ProcessStep[]
            {
                new(Guid.NewGuid(), stepTypeIds[3], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[3], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before),
                new(Guid.NewGuid(), stepTypeIds[3], Entities.Enums.ProcessStepStatusId.TODO, process.Id, before)
            };
        ...
        modifiedProcessSteps.Should().HaveCount(6).And.Satisfy(
            x => processSteps1.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.SKIPPED && x.DateLastChanged != before,
            x => processSteps1.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before,
            x => processSteps1.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before,
            x => processSteps2.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.SKIPPED && x.DateLastChanged != before,
            x => processSteps2.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before,
            x => processSteps2.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before
        );
    }
    ...
    public void SkipProcessStepsExcept_ReturnsExpected()
    {
        ...
        modifiedProcessSteps.Should().HaveCount(3).And.Satisfy(
            x => processSteps0.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.SKIPPED && x.DateLastChanged != before,
            x => processSteps0.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before,
            x => processSteps0.Any(step => step.Id == x.Id) && x.ProcessStepStatusId == Entities.Enums.ProcessStepStatusId.DUPLICATE && x.DateLastChanged != before
        );
    }
    ...
    public void ScheduleProcessSteps_ReturnsExpected()
    {
        ...
        A.CallTo(() => _processStepRepository.CreateProcessStepRange(A<IEnumerable<(ProcessStepTypeId, Entities.Enums.ProcessStepStatusId, Guid)>>._))
            .ReturnsLazily((IEnumerable<(ProcessStepTypeId ProcesssStepTypeId, Entities.Enums.ProcessStepStatusId ProcessStepStatusId, Guid ProcessId)> processStepTypeStatusProcessIds) =>
            {
    ...
    public void FinalizeProcessStep_ReturnsExpected(bool locked)
    {
        ...
        var sut = new ManualProcessStepData(stepTypeIds[1], process, processSteps, _respositories);
        ...
        A.CallTo(() => _processStepRepository.AttachAndModifyProcessSteps(A<IEnumerable<(Guid, Action<ProcessStep>?, Action<ProcessStep>)>>._))
            .MustHaveHappenedOnceExactly();
        A.CallTo(() => _respositories.Attach(process, null))
            .MustHaveHappenedOnceExactly();
